      ...
      <main className="flex-1 max-w-[1440px] mx-auto w-full px-0 md:px-12 pt-3 md:pt-3 pb-24 md:pb-32 animate-in fade-in duration-500">
        <header className="mb-4 md:mb-8 border-b-2 border-[#166534] md:border-navy pb-2.5 md:pb-6 flex items-center justify-between gap-3 px-4 md:px-0 pt-3.5 md:pt-6">
          <div className="flex items-center gap-2 text-left">
            <BackButton className="text-[#166534]/60 hover:text-[#166534] md:text-navy/60 md:hover:text-navy transition-colors flex-shrink-0">
              <IconArrowLeft />
            </BackButton>
            <div>
              <h1 className="text-[16px] md:text-3xl font-serif font-bold tracking-tight text-[#166534] md:text-navy leading-tight whitespace-nowrap ml-2.5 md:ml-4">Recruitments for You</h1>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className={`flex items-center gap-1.5 h-7 md:h-9 px-3 md:px-4 rounded-full transition-all active:scale-95 border ${isFilterApplied ? 'bg-[#166534]/10 text-[#166534] border-[#166534]/20 md:bg-blue-50/50 md:text-blue-600 md:border-blue-200' : 'bg-[#166534]/[0.04] text-[#166534]/60 border-[#166534]/10 hover:bg-[#166534]/[0.06] hover:text-[#166534]/70 md:bg-navy/[0.04] md:text-navy/50 md:border-navy/10 md:hover:bg-navy/[0.06] md:hover:text-navy/70'}`}
            >
              <svg className="w-3 h-3 md:w-3.5 md:h-3.5" viewBox="0 0 24 24" fill="none" stroke={isFilterApplied ? (typeof window !== 'undefined' && window.innerWidth < 768 ? '#166534' : '#2563EB') : 'currentColor'} strokeWidth={isFilterApplied ? '3' : '2'} strokeLinecap="round" strokeLinejoin="round">
              ...
              {isFilterApplied && !isScreeningLoading && (
                <span className="w-1.5 h-1.5 rounded-full bg-[#166534] md:bg-blue-600 ml-0.5" />
              )}
            </button>
            <button
              onClick={() => fetchJobs(true)}
              disabled={isRefreshing || isLoading}
              className={`p-2 rounded-full hover:bg-[#166534]/5 text-[#166534]/60 hover:text-[#166534] md:hover:bg-navy/5 md:text-navy/40 md:hover:text-navy transition-all active:scale-90 ${(isRefreshing || isLoading) ? 'opacity-50' : 'opacity-100'}`}
              title="Refresh Jobs"
            >
          ...
          {isLoading ? (
            ...
            <div className="bg-white border-2 border-gray-100 p-20 text-center rounded-3xl flex flex-col items-center justify-center">
              ...
              {(!userProfile?.qualifications || userProfile.qualifications.length === 0) && (
                <Link
                  href="/profile"
                  className="mt-8 px-6 md:px-10 py-2.5 md:py-3 bg-[#166534] md:bg-navy text-white text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#0f4a24] md:hover:bg-[#06142E] transition-all shadow-xl shadow-green-900/10 md:shadow-navy/20 rounded-lg md:rounded-xl no-underline whitespace-nowrap"
                >
                  setup qualification →
                </Link>
              )}
